...
build_domtree_js: str = """(
    ...
    groups: [
      {
        ...
        palette: ['#ffd54f','#ffca28','#ffc107','#ffb300']
      },
      {
        name: 'label',
        tags: ['label'],
        palette: ['#59d88b', '#0ccf74', '#1ba960', '#1f854d']
      },
      {
        name: 'button',
        tags: ['button'],
        palette: ['#de4d33', '#cf0c0c', '#aa160d', '#87190e']
      },
      {
        name: 'link',
        tags: ['a'],
        palette: ['#ce93d8','#ba68c8','#ab47bc','#9c27b0']
      },
    ],
    defaultPalette: ['#1fb5a6','#229f92','#238a7e','#22756b']
  };

  function highlightElement(element, index, parentIframe = null) {
    const tag = element.tagName.toLowerCase();
    const group = highlightConfig.groups.find(g => g.tags.includes(tag));

    const colors = group ? group.palette : highlightConfig.defaultPalette;
    const baseColor    = colors[index % colors.length];
    const backgroundColor = baseColor + '2A';  // n% opacity

    // Create overlay as before
    ...
    // Calculate position (same as original code)
    ...
    // Label creation (same as original)
    ...
    // Append overlay and label to container
    ...
    const updatePositions = () => {
      ...
      overlay.style.top = `${newRect.top + iframeOffset.y}px`;
      overlay.style.left = `${newRect.left + iframeOffset.x}px`;
      overlay.style.width = `${newRect.width}px`;
      overlay.style.height = `${newRect.height}px`;
       // calculate initial label coords
      const labelWidth  = 20;
      const labelHeight = 16;
      let labelTop  = rect.top + iframeOffset.y + 2;
      let labelLeft = rect.left + iframeOffset.x + rect.width - labelWidth - 2;

      if (rect.width < labelWidth + 4 || rect.height < labelHeight + 4) {
        labelTop  = rect.top + iframeOffset.y - labelHeight  - 2;
        labelLeft = rect.left + iframeOffset.x + rect.width - labelWidth;
      }
  ...
  function buildDomTree(node, parentIframe = null) {
    ...
    if (isInteractiveElement(node)) {
      // only assign highlightIndex to interactable elements that are *not* labels
